...
namespace shadowpoint
{
    public partial class Form2 : Form
    {
        ...
        private bool fullscreenb;
        private Color color;

        public Form2(string path,int slide,bool fullscreen)
        {
            InitializeComponent();
            selpath = path;
            fullscreenb = fullscreen;
            intslide = slide;
        }
                    ...
                    if (Int32.TryParse(slidecof[0], out objectcount))
                    {
                        for (int i = 1; i <= objectcount; i++) // Start at index 1 and iterate up to objectcount
                        {
                            ...
                            if (tempfile[0].ToLower().Contains("image"))
                            {
                                ...
                                if (fullscreenb)
                                {
                                    x = (int)upscaledsize(Screen.PrimaryScreen.Bounds.Width, x);
                                    Console.WriteLine(x);
                                    y = (int)upscaledsize(Screen.PrimaryScreen.Bounds.Height, y);
                                    Console.WriteLine(y);
                                }
                            ...
                            else if (tempfile[0].ToLower().Contains("text"))
                            {
                                ...
                                if (fullscreenb)
                                {
                                    x = (int)upscaledsize(Screen.PrimaryScreen.Bounds.Width, x);
                                    Console.WriteLine(x);
                                    y = (int)upscaledsize(Screen.PrimaryScreen.Bounds.Width, y);
                                    Console.WriteLine(y);
                                }
                                if (tempfile[5] == "")
                                {
        ...
        private float upscaledsize(int defaultsize,int newsize)
        {
            if (defaultsize == 0)
            {
                return 4444;
